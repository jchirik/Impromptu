...
public class FrostedSidebar: UIViewController {
    ...
    public var itemSize:                CGSize                      = CGSize(width: 85.0, height: 85.0)
...
//            circleShape.addAnimation(animation, forKey: nil)
    ...
    private func layoutItems(){
        ...
        for (index, item) in enumerate(itemViews){
            ...
            var frame = CGRect(x: leftPadding, y: topPadding*idx + itemSize.height*idx + topPadding, width:itemSize.width, height: itemSize.height)
            if (index == 4) {
                frame = CGRect(x: leftPadding, y: self.view.bounds.height - itemSize.height - topPadding, width:itemSize.width, height: itemSize.height)
            }
            item.frame = frame
